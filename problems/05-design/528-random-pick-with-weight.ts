 ...
 *   - Solution(int[] w) Initializes the object with the positive integer array w.
 *   - int pickIndex() Returns a random index based on the weights.
 *
 * Use prefix sum + binary search to solve this problem.
 *
 * Example 1:
 *   Input:
 *     ["Solution", "pickIndex"]
 ...
 *   - Build a prefix sum array from weights.
 *   - Generate a random number in [1, totalSum].
 *   - Binary search for the smallest index where prefix[i] >= random number.
 */
...
class Solution {
  ...
  pickIndex(): number {
    // TODO: implement using binary search on prefix sum
    return -1;
  }
}
